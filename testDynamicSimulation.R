...
dy.data = prepare.dy.data()
cat("\n")

test_that("add.object function",{
  new.dy.data = trigger.change(dy.data,padd=1,pdelete=0)
...
test_that("delete.object function",{
  new.dy.data = trigger.change(dy.data,padd=0,pdelete=1)
...
for(i in 1:ndata){
  dy.data = trigger.change(dy.data)
  test_that("",{
